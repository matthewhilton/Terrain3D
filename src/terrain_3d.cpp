...
void Terrain3D::_update_all_collision() {
	for(int i = 0; i < _storage->get_region_count(); i++) {
		_update_collision(i);
	}
}

void Terrain3D::regenerate_collision(Vector3 global_position) {
	// Find the region and only update that one.
	int region_index = _storage->get_region_index(global_position);
	_update_collision(region_index);
}
 ...
 */
void Terrain3D::_update_collision(int region_index) {
	...
	*/
	...
	LOG(DEBUG, "Collision creation time: ", Time::get_singleton()->get_ticks_msec() - time, " ms for region index ", region_index);
}
...
void Terrain3D::_bind_methods() {
	...
	ClassDB::bind_method(D_METHOD("regenerate_collision"), &Terrain3D::regenerate_collision);
